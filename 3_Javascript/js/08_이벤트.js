...
// 아이디가 test1-1인 요소의 onclick 이벤트리스너의
// 이벤트핸들러 제거하기
...
// a few moments later...
test1c.onclik = function() {
  test1c.style.conlor = 'white';
}
//-> onclick에 저장된 값(이벤트 핸들러 함수)이
// 덮어씌워지면서 이전코드(배경색 red)가 사라지는 문제 발생
...
test2.addEventListener("click", function() {
  ...
  if(test2.style.opacity == 0) {// 완젼히 투명해짐
...
const test3 = document.querySelector("#input3");
const box = document.querySelector("#box3");

test3.addEventListener("keyup", function(e) {

  if(e.key == "Enter") {
    box.style.backgroundColor = test3.value;
  }
});
